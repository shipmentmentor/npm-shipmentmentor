...
class shipmentmentor {
		...
		this.shipmentDataSchema = {
			"definitions":{
				...
				"parcel":{
					...
					"additionalProperties": false,
					"required": ["quantity", "name", "length", "width", "height","weight"]					
				}
			},
			"type": "object",
			"properties": {
			...
			"additionalProperties": false,
			"required": ["origin","destination","parcels","courier", "courierEmail"]			
		}
	...
	async sendShipment(payload){
		try {
			...
			return await this.sendApiRequest(payload,"/v1/shipment/create/");
		} catch (error) {
			throw error;
		}
	}	
	...
	async updateShipment(payload){
		try {
			...
			return await this.sendApiRequest(payload,"/v1/shipment/update/");
		} catch (error) {
			throw error;
		}
	}	
	...
	async addTracking(payload){
		try {
			...
			return await this.sendApiRequest(payload,"/v1/trackings/add/");
		} catch (error) {
			throw error;
		}
	}
